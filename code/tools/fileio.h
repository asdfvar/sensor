...
class fio {
   public:
      ...
      bool valid_start_end (int start, int N);

      // Returns a pointer to the acceleration data in x,y,z respectively of sensor 1
      float *get_sens1_ax (void);
      float *get_sens1_ay (void);
      float *get_sens1_az (void);

      // Returns a pointer to the acceleration data in x,y,z respectively of sensor 2
      float *get_sens2_ax (void);
      float *get_sens2_ay (void);
      float *get_sens2_az (void);

      // Returns a pointer to the acceleration data in x,y,z respectively of sensor 3
      float *get_sens3_ax (void);
      float *get_sens3_ay (void);
      float *get_sens3_az (void);
   ...
   private:
      ...
      int N; // Number of lines of data in the kinetisense csv data file
      float **kin_data;
};
